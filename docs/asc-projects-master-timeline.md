...
| Release packaging | **6415 + 6596 release together** (decided 2026-09-23). 6596 is stacked on 6415 on `deployment/dev04` and both were co-tested there — a combined release ships the validated artifact; splitting ZPR out would mean rebasing/re-testing. Combined package inherits ZPR's pre-Oct-1 deadline; ZPR-alone is the break-glass fallback. |
...
>
> **Status update (2026-09-23):** DEV04 refresh done; **QA testing complete for both ASCM Refactor (6415) and ASCM-ZPR (6596); UAT starts 2026-09-24** (per DSM). Only UAT remains before release.
>
> **Release plan (decided 2026-09-23):** **release ASCM Refactor (6415) + ASCM-ZPR (6596) together.** Rationale: on `deployment/dev04`, 6596 is stacked on top of 6415, and both were deployed, executed, and QA-tested as one combined set — a combined release ships exactly the validated artifact and avoids the extra work/risk of splitting ZPR back out. The combined package therefore **inherits ZPR's hard pre-Oct-1 deadline** — if UAT surfaces a Refactor-side issue late, the break-glass fallback is to release ZPR alone to protect Oct 1 (more work given the stacking, so it is a contingency, not the plan). The Refactor is the higher-risk half (shared batch path: ArchiverService/MailerService + DataCorrectionLogic across Pre/Final/Correction); ZPR is a Zipan-only enum add — focus UAT + post-deploy monitoring on the Refactor.
...
| 4 | **Release** | Lead | Release before Oct 1 — **combined with ASCM Refactor (DEVOPS-6415)** | Ship 6415 + 6596 together (stacked + co-tested on DEV04). Ahead of the Oct 1 PRE batch; combined package inherits this hard deadline (ZPR-alone = break-glass fallback). |
...
| **ASCM Refactor + ZPR released (combined)** | — | **before Oct 1** | ★ Hard deadline — Oct 1 PRE batch. 6415 + 6596 shipped together (stacked + co-tested on DEV04). Cristoff/Lead, DEVOPS. | 🔄 Dev + QA testing done; UAT starts 2026-09-24 |
