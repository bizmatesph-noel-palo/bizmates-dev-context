...
|---|---|
| **KISS** | SQL conditions should be readable. Document INTERVAL choices with inline comments. Don't build abstractions (interfaces, factories) for a single implementation — add them only when a second implementation actually appears. **Allocation split arity is now such a case:** R-16 (2026-09-08) makes CIP 1029–1032 a 3-way split, a genuine second implementation alongside the 2-way (CAP + CIP 1028). So an `AllocationFormulaInterface` (2-way now, 3-way in ASCI) is justified — not preemptive. |
...
| **Liskov Substitution (L)** | The `AllocationFormulaInterface` implementations (`TwoWayAllocationFormula` now; `ThreeWayAllocationFormula` in ASCI) must be safely interchangeable at the call site — the engine selects a formula and calls `computeBundleAllocation()` without special-casing arity. A plan with no registered formula is skipped (never run through the wrong formula). |
